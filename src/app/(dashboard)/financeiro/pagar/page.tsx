...
import { CheckCircle, AlertTriangle, Clock, Plus, Save, Trash2, FilePenLine, ArrowDownCircle, History } from 'lucide-react';
...
function alertInfo(dueDate?: string) {
  ...
  if (diff < 0)  return { label: `Vencido há ${Math.abs(diff)}d`, cls: 'text-red-400',    icon: <AlertTriangle className="h-3.5 w-3.5 text-red-400" /> };
  if (diff === 0) return { label: 'Vence hoje',             cls: 'text-orange-400', icon: <AlertTriangle className="h-3.5 w-3.5 text-orange-400" /> };
  if (diff <= 7)  return { label: `Vence em ${diff}d`,     cls: 'text-yellow-400', icon: <Clock className="h-3.5 w-3.5 text-yellow-400" /> };
  return { label: `Vence em ${diff}d`, cls: 'text-muted-foreground', icon: <Clock className="h-3.5 w-3.5" /> };
}
...
const schema = z.object({
  ...
  dueDate: z.string().min(1, 'Vencimento obrigatório para contas a pagar.'),
});
...
export default function ContasAPagarPage() {
  ...
  const load = async () => {
    ...
    const [res, acc, hist] = await Promise.all([
      getPendingTransactions('saida'),
      getAccounts(),
      getTransactionsBySource(['conta_pagar']),
    ]);
  ...
  const categories = useMemo(() =>
    accounts.filter(a => a.type === 'saida').map(a => a.name).sort(), [accounts]);
  ...
  const handlePay = async (t: FinancialTransaction) => {
    ...
    if (result.success) {
      toast({ title: 'Conta baixada!', description: `"${t.description}" registrada como quitada.` });
      load();
    } else {
      toast({ variant: 'destructive', title: 'Erro', description: result.error });
    }
  ...
  const totalPago = historico.filter(t => t.status === 'pago').reduce((s, t) => s + t.amount, 0);
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Registre uma despesa futura. Ao quitar, o status será atualizado no histórico.</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-2">
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Ex: Aluguel, fornecedor, impostos..." {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
          ...
          <TabsContent value="pendentes" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Ordenadas por vencimento. Use o Controle do Caixa para lançar manualmente após o pagamento.</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Todos os lançamentos vinculados a Contas a Pagar, ordenados por data.
                    </CardDescription>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Total quitado: <span className="font-semibold text-foreground">{formatCurrency(totalPago)}</span>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                    ...
                    {loading ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={8} className="h-24 text-center text-muted-foreground">
                          Nenhum histórico de Contas a Pagar encontrado.
                        </TableCell>
                      </TableRow>
                    ) : historico.map(t => (
                      <TableRow key={t.id}>
                        ...
                        <TableCell>
                          ...
                          >
                            {t.status === 'pago' ? 'Quitado' : 'Pendente'}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right font-mono font-semibold text-red-400">
                          - {formatCurrency(t.amount)}
