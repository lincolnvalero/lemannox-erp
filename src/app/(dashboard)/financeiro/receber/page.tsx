...
export default function ContasAReceberPage() {
  ...
  const load = async () => {
    setLoading(true);
    const result = await getPendingTransactions('entrada');
  ...
  const handleReceive = async (t: FinancialTransaction) => {
    ...
    if (result.success) {
      ...
      toast({ title: 'Recebido!', description: `"${t.description}" marcado como recebido.` });
    } else {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3">
        <Card>
          <CardHeader className="pb-2"><CardDescription>Total a Receber</CardDescription></CardHeader>
          <CardContent><p className="text-2xl font-bold text-green-400">{total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardDescription>Recebimentos Pendentes</CardDescription></CardHeader>
          <CardContent><p className="text-2xl font-bold">{transactions.length}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardDescription>Em Atraso</CardDescription></CardHeader>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Recebimentos pendentes ordenados por vencimento</CardDescription>
            </div>
            <Link href="/financeiro/editor?type=entrada">
              <Button size="sm" variant="outline">+ Nova Entrada</Button>
            </Link>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">Nenhum recebimento pendente.</TableCell>
                </TableRow>
              ) : transactions.map(t => {
